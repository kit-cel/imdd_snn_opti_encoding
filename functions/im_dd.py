import torch
import numpy as np
import functions.rrc as rrc  # requires rrc.py to be accessible (e.g., in the same directory) Root Raised Cosine

class IM_DD_CHANNEL:
    def __init__(self,SYMCNT,Baud_Rate,rate,USF,D,channel_length,wavelen,RRC_beta,bias,device):
        self.__SYMCNT = SYMCNT;                 # number of symbols 
        self.__R      = Baud_Rate;              # symbols per second 
        self.__rate   = rate;                   # bits/symbol to convert EbN0 -> EsN0 
        self.__dt     = 1/(USF*Baud_Rate);
        self.__USF    = USF;                    # upsampling-factor for channel simulation
        self.__D      = D;                      # disperion coefficient, in ps/nm/km
        self.__length = channel_length;         # fiber length [m]
        self.__wavelen = wavelen;               # wavelength of transmission [nm]
        self.__rrc_beta = RRC_beta;             # roll-off factor of RRC
        self.__beta = -self.__wavelen**2/(2*np.pi*300000)*self.__D*10**(-27);
        self.__bias = bias                      # Bias added
    ...
    def __generate_H(self):
        g = rrc.get_rrc_ir(self.__SYMCNT * self.__USF + 1, self.__USF, 1, self.__rrc_beta);  # RRC impulse response non-odd countnumber for symmetric filter with clear middle
        g = g[:-1:]
    ...
    def apply(self,symbols,var_t):
        '''bsz_epoch[epoch]
        IN: TX-symbols, noise variance/power [dB]
        OUT: RX-symbols
        '''
        ...
        pre_sqrt = us_symbols * self.__H_without_bias + self.__H_bias   #Apply pulseshaping, bias and chromatic dispersion
        ...
        var_t = 10**((var_t)/10)                                  # dB to linear, remove 3dB bias of implementation
        sigma = torch.sqrt(torch.tensor(var_t))                                       # Calculate sigma (std of noise)
