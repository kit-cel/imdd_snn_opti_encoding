...
import functions.bit                  #requires bit.py
import functions.gray_coder as gray_coder          #requires gray_coder.py
import functions.quantizer as quantizer            #requires quantizer.py
...
#####################################################################
    ...
    def change(self, modulation, m, gray, device,IMDD,LMQ):
        ...
        if(modulation == "QAM"):
            self.__bitmap = functions.bit.BIT_MAPPER(self.__mod_order//2,self.__device)
        elif(modulation == "APSK"):
            self.__bitmap = functions.bit.BIT_MAPPER(self.__mod_order//2,self.__device)
        else:
            self.__bitmap = functions.bit.BIT_MAPPER(self.__mod_order,self.__device)
